...
class MessageTable(Base):
    ...
    metadata_ = Column("metadata", Text)  # JSON stored as text
    ...
    def to_model(self) -> Message:
        return Message(
            ...
            metadata=cast(list[dict[str, Any]], json.loads(self.metadata_) if self.metadata_ else []),
        )


class SQLiteMessageDatabase(MessageDatabase):
    ...
    async def get_messages_by_conversation_id(self, conversation_id: str) -> list[Message]:
        async with self.make_session() as session:
            try:
                ...
                return [m.to_model() for m in result.scalars().all()]
            except Exception as e:
    ...
    async def get_message_by_id(self, message_id: str) -> Message:
        async with self.make_session() as session:
            try:
                msg: MessageTable | None = await session.get(MessageTable, message_id)
                if msg is None:
    ...
    async def delete_message(self, message_id: str) -> bool:
        async with self.make_session() as session:
            async with session.begin():
                try:
                    msg = await session.get(MessageTable, message_id)
                    if msg:
