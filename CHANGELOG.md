...
New `POST /api/v1/rag/analyze-document` endpoint, a `scripts/batch-analyze.py` helper, and an Explorer tab for running a caller-defined analysis over every document in a KB and collecting the results as a CSV table. Sibling to the interactive expand-context feature: non-streaming, per-document, script-friendly.
...
**Web UI** — Explorer gains a fourth tab covering the same workflow without files or a terminal. Documents are pasted or uploaded as an identifier list (same blank-line and `#`-comment rules as the script, with a `source_file` / `document_id` selector). Questions are built as rows — type (`text` / `boolean` / `number` / `choice`), question text, and for `choice` a comma-separated list of allowed answers — from which the prompt and the JSON Schema are generated together. That removes the class of failure where the two drift apart: the schema carries the answer format, so no generated format hints are needed and nothing has to be translated when questions are written in another language. Generated schemas always mark every property `required` with `additionalProperties: false`, making the strict-mode warnings the script reports structurally impossible. The question set persists in `localStorage`, and `prompt.txt` / `schema.json` export for a later headless run.

**Answering "not stated"** — schema field names are positional (`question1`…`questionN`) and not user-editable: they exist only to line the questions up with the schema, and the exports carry the readable form instead (CSV headers append a slug of the question, `question3_how_many_towers_are_mentioned`, so columns stay in question order; the JSON export carries the full question text alongside the results). Number fields are emitted as `["number", "null"]` so a document that does not state the value has a legal answer — without it, strict decoding forces a number and the honesty rule turns "couldn't tell" into a confident `0`. Choice rows are seeded with a `yes, no, unclear` suggestion for the same reason, editable and deletable; nothing is appended to an enum behind the user's back. Past ten questions in one pass the tab warns that per-answer accuracy degrades, without blocking — the real limit is model-dependent.

**Prompt transparency** — a collapsible preview shows the complete prompt the model receives: the backend template with `{user_prompt}` and `{schema}` substituted live. The template is fetched from the new `GET /api/v1/rag/analyze-prompt-template` rather than duplicated in the frontend, so a `batch_analyze.custom.md` override displays as-is instead of silently drifting from what is actually sent.

**UI preflight** — Verify reports backend/model/`num_ctx`, the derived per-document char budget, duplicate identifiers dropped, and which identifiers are missing from the KB or over budget, all before a single LLM call. `POST /api/v1/rag/document-stats` gained an optional `id_field` (`source_file` by default) so this check works for `document_id` batches too; the expand-context popover is unaffected. Requests are chunked to respect the endpoint's 100-identifier cap.
...
New pill in the `ChatActionBar` (next to the filter action) lets the user skip the RAG pipeline entirely for the next message. When on, the backend bypasses semantic search, BM25, query rewriting, HyDE, query expansion, and reranking — the LLM answers using only the prior conversation history and its general knowledge.

**Use case:** meta-questions ("what did I ask last?"), casual chat, or when the user wants a pure LLM response without vector-search noise mixing in.

**Behaviour:**

- **Per-message flag**, not persisted server-side. Toggle stays on for successive messages until the user turns it off; resets when starting a new conversation.
- **Orthogonal to chat filters** — if both are set for a message, filters are silently ignored (no retrieval to filter). The frozen filter chips still render for context in continuation conversations.
- **Follow-up questions** are still generated by the LLM in the same JSON schema; sources on the response are always empty and the citation chip strip is not rendered.

**Backend:**

- `MessageInput.chat_only: bool = False` (new field on `POST /api/v1/messages/stream`).
- `QueryWithContext.chat_only` threads to `RAG.answer_stream`, which short-circuits at the top: builds `[SYSTEM (chat-only prompt), *history, user_query]` and streams the LLM response directly. No retrievers are called, no query preprocessing runs.
- New prompt file `prompts/chat_only.default.md` — mirrors the `system_prompt.*.md` pattern. Admin can drop a `chat_only.custom.md` override (gitignored) to customise the chat-only behaviour without touching code. Empty falls back to the standard system prompt.
- `RAG.__init__` accepts `chat_only_system_prompt: str = ""`; loaded once at startup in `main.py::_load_chat_only_prompt()`.

**Frontend:**

- `ChatOnlyAction` component in `chat-action-bar.tsx`, a simple toggle pill (no popover). Amber-tinted when active to visually distinguish from the blue filter chips.
- `useMessaging` gains `chatOnly` state + `setChatOnly` setter. The value is included in the request body only when `true`. Reset on `createNewConversation`.
