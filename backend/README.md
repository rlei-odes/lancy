...
This package (`lancy`) contains the supporting Python modules that run the RAG pipeline for the PrimePack AG sustainability use case.
...
cd backend
BACKEND=openai python -m lancy.main

# Or with uvicorn for hot reload during development
uvicorn lancy.main:app --reload --port 8080
```
...
```bash
# Default (OpenAI)
python -m lancy.feature0_baseline_rag
...
RESET_VS=1 python -m lancy.feature0_baseline_rag
