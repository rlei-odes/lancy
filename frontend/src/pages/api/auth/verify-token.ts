...
function checkGroupMembership(payload: JWTPayload, allowedGroups: string[]): boolean {
    ...
    const combined = [...groups, ...roles].map((g) => g.toLowerCase());
    return allowedGroups.some((g) => combined.includes(g.toLowerCase()));
}
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
    ...
    const authToken = await signToken("user", appPassword, ttl);
